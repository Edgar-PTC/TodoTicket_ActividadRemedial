...
wompiController.generarToken = async(req, res) => {
    try {
        ...
        if(!response.ok){
            ...
            return res.status(500).json({ message: "Error interno del servidor generando token: ", error });
        }

        const data = await response.json();
        return res.status(200).json(data);
    } catch (error) {
        console.log("Error en wompiController: ", error);
...
wompiController.paymentTest = async (req, res) => {
    try {
        ...
        if(!response.ok){
            const error = await response.json();
            return res.status(500).json({ message: "Error interno del servidor pagando: ", error });
        }
