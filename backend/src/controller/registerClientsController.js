...
registerClientsController.insert = async (req, res) => {
    try {
        ...
        const exist = await clientsModel.findOne({ email });
        if(exist){
        ...
        const newUser = clientsModel({ name, lastName, email, password: passwordHash, isVerified: false });
        await newUser.save();
...
registerClientsController.verifyCode = async(req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Client verified" });
    } catch (error) {
        console.log("Error en registerClient: ", error);
        return res.status(500).json({ message: "Error interno del servidor" });
    }
